...
template<typename T>
Option<std::vector<T>> make_option(std::string const& _str, std::vector<T> const& _selection, std::string const& _description) {
...
inline Option<std::vector<std::string>> make_option(std::string const& _str, std::vector<char const*> const& _selection, std::string const& _description) {
	std::vector<std::string> selection;
	for (auto c : _selection) {
		selection.push_back(c);
	}
	return make_option<std::string>(_str, selection, _description);
}
